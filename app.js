...
function populateFilters(exercises){
  ...
  typeContainer.innerHTML = ''; subgroupContainer.innerHTML = '';
  ...
  types.forEach(t=>{
    const l = document.createElement('label'); l.innerHTML=`<input type="checkbox" value="${t}"> ${t}`; typeContainer.appendChild(l);
    l.querySelector('input').addEventListener('change', applyFilters);
  });
  subgroups.forEach(s=>{
    const l = document.createElement('label'); l.innerHTML=`<input type="checkbox" value="${s}"> ${s}`; subgroupContainer.appendChild(l);
    l.querySelector('input').addEventListener('change', applyFilters);
  });
}
...
document.getElementById('close-tutorial-modal-btn')?.addEventListener('click', ()=>{
  document.getElementById('tutorial-modal').classList.add('hidden');
  document.getElementById('tutorial-video').src='';
});
...
document.getElementById('clear-filters')?.addEventListener('click', ()=>{
  ...
  document.querySelectorAll('#type-options input').forEach(cb=>cb.checked=false);
  document.querySelectorAll('#subgroup-options input').forEach(cb=>cb.checked=false);
...
// ---------- Simulación de carga ----------
async function loadTutorials(){
  // Aquí normalmente harías fetch de supabase
  tutorialsData=[
    {id:1,name:'Press Banca',type:'Fuerza',subgroup:'Pecho',exercise_tutorials:[{video_url:'https://www.youtube.com/embed/dQw4w9WgXcQ',cues:'Mantén la espalda recta'}]},
    {id:2,name:'Sentadilla',type:'Fuerza',subgroup:'Piernas',exercise_tutorials:[{video_url:'https://www.youtube.com/embed/dQw4w9WgXcQ',cues:'Rodillas alineadas'}]},
    {id:3,name:'Curl Bíceps',type:'Aislamiento',subgroup:'Brazos',exercise_tutorials:[{video_url:'https://www.youtube.com/embed/dQw4w9WgXcQ',cues:'Controla el movimiento'}]}
  ];
